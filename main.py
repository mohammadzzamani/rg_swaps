...
def to_nominal(data, col_name, values=[], nominal_col_name=None):
    ...
    data = data[sdf[col_name].isin(values)]
...
def split_train_test(data, date_col):
    ...
    train_max_date = datetime.date(2017,12,01)
...
def simple_learning_model(xtrain, ytrain, xtest, ytest, ratio = 0.5, rg_model = '...'):
    ...
    # print("f1_score: %.2f" % metrics.f1_score(ytest, pred, average='binary'))
...
def prepare_data(data, date_col='dt'):
    ...
    # data['datediff'] = data[last_rg_dt] - data[date_col]
    ...
    data = to_nominal(data,'trouble_code_desc', trouble_codes)
...
def print_detailed_outcomes(testdf, ypred, outputfilename=None):
    ...
    # yAll = U.stack(ypred.transpose().reshape((len(ypred),1)), ytest, axis=1)
    # print ( type(yAll))
    # print (yAll.shape)
    # ydf = pd.DataFrame(yAll, columns = ['true_value', 'prediction'] )
...
for key, sdf in sdf_dict.iteritems():
    ...
    ypred, ypred_proba = simple_learning_model(xtrain.values, ytrain.values, xtrain.values, ytrain.values, (ratio-0.1)/0.9 , key)
